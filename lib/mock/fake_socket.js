...
var MAX_READ_SIZE = 8000;
FakeSocket.prototype.read = function (callback) {
  var self = this;
  
  var totalBuffer = Buffer.concat(self._buffer);
  
  if (totalBuffer.length > MAX_READ_SIZE) {
    self._buffer = [totalBuffer.slice(MAX_READ_SIZE)];
    totalBuffer = totalBuffer.slice(0, MAX_READ_SIZE);
  } else {
    self._buffer = [];
  }

  callback(null, totalBuffer.toString('hex'));
};
